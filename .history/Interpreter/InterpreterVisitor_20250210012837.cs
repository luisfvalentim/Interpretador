...
public override object VisitPrimary(CSubsetParser.PrimaryContext context)
{
    ...
    else if (context.ID() != null)
    {
        ...
        // 🔹 Se for um array, acessamos corretamente os elementos `arr[index]`
        if (context.ChildCount > 3 && context.GetChild(1).GetText() == "[") // ✅ Se `arr[index]` está presente
        {
            object indexObj = Visit(context.GetChild(2)); // ✅ Obtém o índice do array
        ...
        return memory[varName]; // ✅ Retorna a variável normalmente
    }
