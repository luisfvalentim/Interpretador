...
public override object VisitAssignmentExpression(CSubsetParser.AssignmentExpressionContext context)
{
    ...
    // 🔹 Se for um array, faz atribuição corretamente
    if (context.GetChildCount() > 3) // ✅ Verifica se há um índice (arr[index] = valor)
    {
    ...
    // ✅ Marca a variável como inicializada ANTES de chamar `CheckInitialization`
    memory[varName] = value;

    // ✅ Agora podemos verificar inicialização corretamente
...
public override object VisitPrimary(CSubsetParser.PrimaryContext context)
{
    ...
    else if (context.ID() != null)
    {
        ...
        // 🔹 Se for um array, acessamos corretamente os elementos
        if (context.GetChildCount() > 3) // ✅ Verifica se `arr[index]` está sendo usado
        {
