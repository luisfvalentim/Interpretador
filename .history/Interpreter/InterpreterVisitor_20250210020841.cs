    ...
    if (functionName == "itoa")
{
    ...
    {
        return intValue.ToString(); // ✅ Agora converte corretamente apenas inteiros
    }
    else if (value is float) // ❌ Se for float, sugere usar `ftoa`
    {
        throw new Exception($"Erro: itoa só pode converter inteiros para string, mas recebeu Float. Use `ftoa` para converter floats.");
    }
    else
    {
