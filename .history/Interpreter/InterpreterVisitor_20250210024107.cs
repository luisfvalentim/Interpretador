...
{
    string varName = context.ID(0).GetText(); // ✅ Corrige acesso ao nome da variável
    string type = context.type().GetText();
    object? value = null;

    // 🔹 Se for um array, inicializa corretamente
    if (context.NUMBER() != null) // ✅ Verifica se um número (tamanho do array) está presente
    {
    ...
    // 🔹 Se houver uma atribuição, avalia a expressão
    if (context.expression() != null)
    {
        ...
        else if (type == "float" && value is int intValue)
        {
            value = (float)intValue; // ✅ Converte `int` para `float` se necessário
        }
        else if (type == "char" && value is string strValue && strValue.Length == 1)
        {
            value = strValue[0]; // ✅ Converte `string` de um caractere para `char`
        }
        else if (type == "char" && value is not char)
        {
            throw new Exception($"Erro: Variável '{varName}' esperava um `char`, mas recebeu {value?.GetType().Name ?? "null"}.");
        }
        else if (type == "string" && value is not string)
        {
            value = value.ToString(); // ✅ Converte qualquer tipo para `string`
        }
    }
...
public override object VisitPrimary(CSubsetParser.PrimaryContext context)
{
    if (context.NUMBER() != null)
    {
        ...
        // ✅ Garante que `10` continua `int` e `3.14` seja `float`
 ...
 // 🔹 Verifica presença de colchetes `[]`
        {
...
public override object VisitForStatement(CSubsetParser.ForStatementContext context)
{
    ...
    if (context.declaration() != null)
    {
        ...
        varName = context.declaration().ID(0)?.GetText(); // ✅ Corrige acesso ao nome da variável
    }
    else if (context.assignmentExpression(0) != null)
    {
        Visit(context.assignmentExpression(0));
        varName = context.assignmentExpression(0).GetChild(0)?.GetText(); // ✅ Obtém o nome corretamente
    }
    ...
    while (context.expression() != null && Convert.ToBoolean(Visit(context.expression()))) // ✅ Avaliação segura da condição
    {
        try
        {
