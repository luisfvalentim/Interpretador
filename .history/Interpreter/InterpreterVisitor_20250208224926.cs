        ...
        public override object VisitFunctionCall(CSubsetParser.FunctionCallContext context)
{
    ...
    if (functionName == "printf")
    {
        ...
        while ((formattedString.Contains("%d") || formattedString.Contains("%f")) && argIndex < args.Count)
        {
            ...
            {
                formattedString = formattedString.Replace("%d", args[argIndex].ToString(), 1); // Substitui apenas a primeira ocorrência
            }
            else if (formattedString.Contains("%f") && args[argIndex] is float)
            {
                formattedString = formattedString.Replace("%f", ((float)args[argIndex]).ToString("F2"), 1); // Substitui apenas a primeira ocorrência
            }
            argIndex++;
        }
