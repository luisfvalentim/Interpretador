...
public override object VisitReturnStatement(CSubsetParser.ReturnStatementContext context)
{
    ...
    if (context.expression() != null)
    {
        returnValue = Visit(context.expression()); // ✅ Captura o valor de retorno corretamente
    }

    Console.WriteLine($"DEBUG: Retornando valor da função: {returnValue}"); // 🔥 Depuração

    throw new ReturnException(returnValue); // ✅ Lançamos uma exceção para interromper a execução da função
}
