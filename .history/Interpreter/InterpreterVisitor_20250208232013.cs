...
public override object VisitForStatement(CSubsetParser.ForStatementContext context)
{
    ...
    {
        Visit(context.declaration()); // Processa a declaração de variável (armazenando na memória)
    ...
    // Se a variável não estiver na memória, significa que não foi declarada corretamente
    if (varName == null || !memory.ContainsKey(varName))
    {
        throw new Exception($"Erro: Variável de controle '{varName}' não encontrada.");
    }

    // Debug: Verifica o valor da variável antes de entrar no loop
