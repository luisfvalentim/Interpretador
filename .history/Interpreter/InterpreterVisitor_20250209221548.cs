...
{
    string varName = context.ID().GetText();
    object value = Visit(context.expression()); // ✅ Correto para expressões normais

    // 🔹 Verifica se é um array (ex: arr[2] = 10;)
    if (context.children.Count == 4)  
    {
        int index = Convert.ToInt32(Visit(context.expression(0))); // ✅ Obtém índice corretamente
        value = Visit(context.expression(1)); // ✅ Pegando o valor certo
        ...
        if (memory.ContainsKey(varName) && memory[varName] is object[] array)
        {
            ...
            array[index] = value; // ✅ Agora atribui corretamente
            return null;
        }
        else
        {
...
public override object VisitPrimary(CSubsetParser.PrimaryContext context)
{
    ...
    else if (context.ID() != null)
    {
        ...
        if (context.children.Count == 4)  
        {
            int index = Convert.ToInt32(Visit(context.expression())); // Obtém o índice
            ...
            if (memory.ContainsKey(varName) && memory[varName] is object[] array)
            {
                ...
                return array[index]; // Retorna o valor do array
            }
            else
            {
                throw new Exception($"Erro: '{varName}' não é um array.");
