...
{
    // Inicialização do for (ex: int i = 0; ou i = 0;)
    ...
    {
        Visit(context.assignmentExpression(0)); // Ajustado para acessar corretamente a inicialização
    }
    ...
    while (context.expression() == null || Convert.ToBoolean(Visit(context.expression())))
    {
        ...
        // Incremento do for (ex: i = i + 1;)
        if (context.assignmentExpression() != null && context.assignmentExpression().Length > 1)
        {
            Visit(context.assignmentExpression(1)); // Ajustado para acessar corretamente o incremento
        }
    }
