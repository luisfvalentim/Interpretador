    ...
    public class InterpreterVisitor : CSubsetBaseVisitor<object>
    {
        ...
        public override object VisitIfStatement(CSubsetParser.IfStatementContext context)
        {
            ...
            if (condition)
            {
                ...
                return null; // ✅ Sai da função para evitar avaliar `else if` ou `else`
            }

            // ✅ Percorre todos os `else if`
            for (int i = 0; i < context.children.Count; i++)
            {
                if (context.GetChild(i).GetText() == "else" && context.GetChild(i + 1).GetText() == "if")
                {
                    bool elseIfCondition = Convert.ToBoolean(Visit(context.expression(i / 2 + 1))); // Ajusta índice da expressão do `else if`

                    if (elseIfCondition)
                    {
                        Visit(context.block(i / 2 + 1)); // ✅ Executa o bloco do `else if`
                        return null; // ✅ Sai da função para evitar avaliar `else`
                    }
                }
            }

            // ✅ Se existir `else`, executa
            if (context.block().Length > 1 && context.GetChild(context.ChildCount - 2).GetText() == "else")
            {
                Visit(context.block(context.block().Length - 1)); // ✅ Executa `else`
            }
        ...
        public override object VisitSwitchStatement(CSubsetParser.SwitchStatementContext context)
        {
            ...
            bool caseMatched = false; // Verifica se um `case` já foi executado

            foreach (var caseStmt in context.caseStatement())
            {
                object caseValue = Visit(caseStmt.expression()); // Suporte a expressões

                if (switchValue.Equals(caseValue) || caseMatched)
                {
                    caseMatched = true; // Marca que um `case` foi ativado
                    foreach (var stmt in caseStmt.statement())
                    {
                        Visit(stmt);
                    }

                    // ✅ Se encontrar um `break`, sai do switch
                    if (caseStmt.breakStatement() != null)
                        break;
                }
            }

            // ✅ Se nenhum `case` foi ativado e existir `default`, executa-o
