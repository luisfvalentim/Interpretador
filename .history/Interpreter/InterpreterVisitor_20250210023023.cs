...
public override object VisitExpression(CSubsetParser.ExpressionContext context)
{
    ...
    {
        string unionVar = context.GetChild(0).GetText();
        string member = context.GetChild(2).GetText();

        if (!memory.ContainsKey(unionVar))
        {
            throw new Exception($"Erro: Variável '{unionVar}' não declarada.");
        }

        if (!(memory[unionVar] is Dictionary<string, object> unionData))
        {
            throw new Exception($"Erro: '{unionVar}' não é uma union.");
        }

        // ✅ Verifica se outro membro já foi definido e lança erro
        foreach (var key in unionData.Keys)
        {
            if (!key.Equals(member) && unionData[key] != null)
            {
                throw new Exception($"Erro: Apenas um membro da union '{unionVar}' pode ser usado por vez.");
            }
        }

        return unionData[member];
    }
