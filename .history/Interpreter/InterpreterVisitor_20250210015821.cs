...
public override object VisitFunctionCall(CSubsetParser.FunctionCallContext context)
{
    ...
    if (functionName == "printf")
    {
        ...
        while ((formattedString.Contains("%d") || formattedString.Contains("%f") || formattedString.Contains("%s") || formattedString.Contains("%c")) && argIndex < args.Count)
        {
            ...
            {
                formattedString = formattedString.Replace("%f", string.Format("{0:F2}", args[argIndex]), 1);
            }
            else if (formattedString.Contains("%s"))
            {
                formattedString = formattedString.Replace("%s", args[argIndex].ToString(), 1);
            }
            else if (formattedString.Contains("%c"))
            {
                formattedString = formattedString.Replace("%c", args[argIndex].ToString(), 1);
            }
            argIndex++;
        }
    ...
    if (functionName == "itoa")
    {
        ...
        return value is int intValue ? intValue.ToString() : throw new Exception("Erro: itoa só pode converter inteiros para string.");
    }

    // ✅ `ftoa` - Converte float para string
    if (functionName == "ftoa")
    {
        ...
        return value is float floatValue ? floatValue.ToString(System.Globalization.CultureInfo.InvariantCulture) : throw new Exception("Erro: ftoa só pode converter floats para string.");
    }
