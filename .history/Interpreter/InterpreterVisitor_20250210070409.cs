...
private bool mainExecuted = false; // ✅ Evita múltiplas execuções da `main`

public override object VisitFunction(CSubsetParser.FunctionContext context)
{
    string functionName = context.ID().GetText();

    Console.WriteLine($"🔹 Função detectada: {functionName}");

    // ✅ Armazena a função no dicionário
    functions[functionName] = context;

    // ✅ Se a função for `main`, executa automaticamente (somente uma vez)
    if (functionName == "main" && !mainExecuted)
    {
        mainExecuted = true;
        Console.WriteLine("✅ Executando função `main`...");
        Visit(context.block());
    }
...
public override object VisitBlock(CSubsetParser.BlockContext context)
{
    ...
    {
        Visit(statement); // ✅ Visita cada statement dentro do bloco `{ ... }`
    }
...
public override object VisitPreprocessorDirective(CSubsetParser.PreprocessorDirectiveContext context)
{
    ...
    switch (directive)
    {
        case "include":
            ...
            // 🔹 Verifica `#include "arquivo.h"` ou `#include <arquivo.h>`
            if (context.GetChild(2).GetText().StartsWith("\"")) 
            {
                fileName = context.GetChild(2).GetText().Trim('"'); // `#include "arquivo.h"`
            }
            else if (context.GetChild(2).GetText() == "<") 
            {
                fileName = context.GetChild(3).GetText().Trim('>'); // `#include <stdio.h>`
            }
            else
            {
            ...
            Console.WriteLine($"✅ Biblioteca incluída: {fileName}");
            ...
            // 🔹 Se for `stdio.h`, registra funções padrões
            if (fileName == "stdio.h")
            {
                RegisterStdioFunctions();
            }

            // 🔹 Se for `stdlib.h`, registra funções de conversão
