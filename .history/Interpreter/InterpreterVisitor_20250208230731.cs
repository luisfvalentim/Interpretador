        ...
        public override object VisitFunctionCall(CSubsetParser.FunctionCallContext context)
{
    ...
    if (functionName == "printf")
    {
        ...
        // Remove as aspas da string (caso existam)
        ...
        // Substituir %d, %f, %s e %c pelos valores fornecidos
        while (argIndex < args.Count)
        {
            if (formattedString.Contains("%d") && args[argIndex] is int)
            {
                formattedString = formattedString.Replace("%d", args[argIndex].ToString()); // Substitui apenas a primeira ocorrência
            }
            else if (formattedString.Contains("%f") && args[argIndex] is float)
            {
                formattedString = formattedString.Replace("%f", ((float)args[argIndex]).ToString("F2")); // Substitui apenas a primeira ocorrência
            }
            else if (formattedString.Contains("%s") && args[argIndex] is string)
            {
                formattedString = formattedString.Replace("%s", args[argIndex].ToString()); // Substitui apenas a primeira ocorrência
            }
            else if (formattedString.Contains("%c") && args[argIndex] is char)
            {
                formattedString = formattedString.Replace("%c", args[argIndex].ToString()); // Substitui apenas a primeira ocorrência
            }
