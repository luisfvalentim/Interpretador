...
public override object VisitReturnStatement(CSubsetParser.ReturnStatementContext context)
{
    if (context.expression() != null)
    {
        object returnValue = Visit(context.expression()); // ✅ Captura o valor correto
        Console.WriteLine($"DEBUG: Retornando valor da função: {returnValue}"); // 🔥 Depuração
        return returnValue; // ✅ Agora retorna corretamente o valor
    }

    Console.WriteLine("DEBUG: Retorno sem valor, retornando null"); // 🔥 Depuração
    return null; // Se não houver expressão, retorna `null`
}
...
public override object VisitFunctionCall(CSubsetParser.FunctionCallContext context)
{
    ...
    // ✅ Executa a função e captura o valor retornado
    object returnValue = Visit(functionContext.block());

    // 🔥 Debug: Verificar se a função realmente retornou um valor
    Console.WriteLine($"DEBUG: Função '{functionName}' retornou: {returnValue}");

    // ✅ Se a função retornar null, substituímos por `0`
    if (returnValue == null)
    {
        returnValue = 0;
        Console.WriteLine($"DEBUG: Retorno da função '{functionName}' era null, alterado para 0.");
    }

    // ✅ Restaura a memória original após a execução da função
