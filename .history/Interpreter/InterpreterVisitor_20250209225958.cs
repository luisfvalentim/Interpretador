...
public override object VisitReturnStatement(CSubsetParser.ReturnStatementContext context)
{
    if (context.expression() != null)
    {
        return Visit(context.expression()); // ✅ Retorna o valor correto
    }
...
public override object VisitFunctionCall(CSubsetParser.FunctionCallContext context)
{
    ...
    // ✅ Armazena os argumentos na memória antes da execução da função
    ...
    for (int i = 0; i < parameters?.Length; i++)
    {
        ...
        // ✅ Converte corretamente para `int` ou `float` conforme necessário
        if (memory.ContainsKey(paramName))
        {
            object currentType = memory[paramName];

            if (currentType is float)
            {
                argValue = Convert.ToSingle(argValue);
            }
        ...
        // 🔥 Debugging para verificar valores dos parâmetros
        Console.WriteLine($"DEBUG: Parâmetro '{paramName}' armazenado como {memory[paramName]}");
    }
