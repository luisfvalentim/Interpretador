    ...
    switch (directive)
    {
        case "include":
            ...
            else if (context.GetChild(2).GetText() == "<") // `#include <arquivo.h>`
            {
                ...
                // 🔹 Garante que o nome do arquivo termina em `.h`
            ...
            {
                includedLibraries.Add("stdlib.h");
                RegisterStdlibFunctions();
            }

            // 🔹 Se for `math.h`, registra funções matemáticas (`sin`, `cos`, etc.)
            if (fileName == "math.h")
            {
                includedLibraries.Add("math.h");
...
{
    functions["printf"] = null;
    functions["scanf"] = null;
    functions["puts"] = null;
    functions["gets"] = null;
}
