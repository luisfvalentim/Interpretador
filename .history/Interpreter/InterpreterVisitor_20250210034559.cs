...
namespace Interpretador.Interpreter
{
    public class InterpreterVisitor : CSubsetBaseVisitor<object>
    {
        ...
        private HashSet<string> includedLibraries = new HashSet<string>(); // Guarda bibliotecas incluídas
...
public override object VisitFunctionCall(CSubsetParser.FunctionCallContext context)
{
    ...
    // 🔹 Verifica se funções padrão estão sendo usadas sem `#include`
    if ((functionName == "printf" || functionName == "scanf" || functionName == "puts" || functionName == "gets") 
        && !includedLibraries.Contains("stdio.h"))
    {
        throw new Exception($"❌ Erro: Função '{functionName}' usada sem `#include <stdio.h>`.");
    }

    if ((functionName == "atoi" || functionName == "atof" || functionName == "itoa" || functionName == "ftoa") 
        && !includedLibraries.Contains("stdlib.h"))
    {
        throw new Exception($"❌ Erro: Função '{functionName}' usada sem `#include <stdlib.h>`.");
    }
    ...
    if (functionName == "printf")
    {
        ...
        while ((formattedString.Contains("%d") || formattedString.Contains("%f") || formattedString.Contains("%s") || formattedString.Contains("%c")) 
               && argIndex < args.Count)
        {
    ...
    // ✅ `atoi`, `atof`, `itoa`, `ftoa` - Conversão de tipos
    if (functionName == "atoi")
    {
        return Visit(context.expression(0)) is string str && int.TryParse(str, out int result) ? result : throw new Exception("Erro: atoi não pôde converter a string para inteiro.");
    }
    if (functionName == "atof")
    {
        return Visit(context.expression(0)) is string str && float.TryParse(str, System.Globalization.CultureInfo.InvariantCulture, out float result) ? result : throw new Exception("Erro: atof não pôde converter a string para float.");
    }
    ...
    // ✅ Verifica se a função foi definida pelo usuário
...
{
    string directive = context.GetChild(1).GetText(); // Obtém `include`, `define`, etc.
    ...
    switch (directive)
    {
        case "include":
            ...
            // 🔹 Verifica se `#include "arquivo.h"` ou `#include <arquivo.h>`
            if (context.GetChild(2).GetText().StartsWith("\"")) // `#include "arquivo.h"`
            {
                fileName = context.GetChild(2).GetText().Trim('"');
            }
            else if (context.GetChild(2).GetText() == "<") // `#include <arquivo.h>`
            {
                fileName = context.GetChild(3).GetText().Trim('>'); // 🔹 Obtém `stdio.h`
            }
            else
            {
            ...
            Console.WriteLine($"✅ Incluindo arquivo: {fileName}");

            // 🔹 Registra a biblioteca incluída
            includedLibraries.Add(fileName);

            // 🔹 Se for `stdio.h`, registra funções padrões (printf, scanf, etc.)
            if (fileName == "stdio.h")
            {
                RegisterStdioFunctions();
            }

            // 🔹 Se for `stdlib.h`, registra funções de conversão (atoi, atof, etc.)
        ...
        case "define":
            ...
            string macroValue = string.Join(" ", context.GetText().Split(' ').Skip(3)); // 🔹 Obtém tudo depois do nome da macro
        ...
        default:
            throw new Exception($"❌ Erro: Diretiva pré-processador '{directive}' não reconhecida.");
    }
...
{
    functions["atoi"] = new CSubsetParser.FunctionContext(null, 0);
    functions["atof"] = new CSubsetParser.FunctionContext(null, 0);
    functions["itoa"] = new CSubsetParser.FunctionContext(null, 0);
    functions["ftoa"] = new CSubsetParser.FunctionContext(null, 0);
}
