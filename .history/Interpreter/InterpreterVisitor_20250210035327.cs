...
public override object VisitPreprocessorDirective(CSubsetParser.PreprocessorDirectiveContext context)
{
    ...
    switch (directive)
    {
        case "include":
            ...
            if (fileName == "stdio.h") { includedLibraries.Add("stdio.h"); RegisterStdioFunctions(); }
            if (fileName == "stdlib.h") { includedLibraries.Add("stdlib.h"); RegisterStdlibFunctions(); }
            if (fileName == "math.h") { includedLibraries.Add("math.h"); RegisterMathFunctions(); }
            break;

        case "define":
            if (context.ChildCount < 3)
            {
                throw new Exception("❌ Erro: `#define` deve ter pelo menos um nome.");
            }

            string macroName = context.GetChild(2).GetText();
            string macroValue = context.ChildCount > 3 ? context.GetChild(3).GetText() : "1";
        ...
        case "ifndef":
            string ifndefName = context.GetChild(2).GetText();

            if (macros.ContainsKey(ifndefName))
            {
        ...
        default:
            throw new Exception($"❌ Erro: Diretiva de pré-processador '{directive}' não reconhecida.");
    }
...
public override object Visit(IParseTree tree)
{
    if (ignoreCodeBlock)
    {
        return null; // ❌ Ignora código dentro de `#ifndef` e `#endif`
    }
