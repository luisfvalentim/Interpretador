...
public override object VisitDeclaration(CSubsetParser.DeclarationContext context)
{
    ...
    if (context.GetChildCount() > 3 && context.GetChild(2).GetText() == "[") 
    {
        ...
        memory[varName] = array; // ✅ Agora o array está corretamente armazenado

        return null;
    }

    // 🔹 Se for variável normal, inicializa corretamente
    if (context.expression() != null)
    {
        value = Visit(context.expression());
    }

    memory[varName] = value; // ✅ Armazena a variável na memória
...
public override object VisitAssignmentExpression(CSubsetParser.AssignmentExpressionContext context)
{
    ...
    if (context.ChildCount > 3 && context.GetChild(1).GetText() == "[") // ✅ Confirma `arr[index]`
    {
        object indexObj = Visit(context.GetChild(2)); // ✅ Obtém o índice
        ...
        public override object VisitAdditiveExpression(CSubsetParser.AdditiveExpressionContext context)
{
    ...
    // 🔹 Verifica se a operação envolve tipos incompatíveis
    if (left is string || right is string || left is char || right is char || left is object[] || right is object[])
    {
        throw new Exception($"Erro: Operação inválida entre {left.GetType().Name} e {right.GetType().Name}.");
    }

    // 🔹 Se houver um float, a operação deve ser em ponto flutuante
    ...
    // 🔹 Caso contrário, opera com inteiros normalmente
...
public override object VisitPrimary(CSubsetParser.PrimaryContext context)
{
    ...
    else if (context.ID() != null)
    {
        ...
        // 🔹 Verifica se estamos acessando um índice do array `arr[index]`
        if (context.ChildCount > 3 && context.GetChild(1).GetText() == "[") // ✅ Verifica presença de colchetes
        {
            // ✅ Obtém o índice corretamente
            object indexObj = Visit(context.expression());

            if (!(indexObj is int index)) // 🔹 Garante que é um número inteiro
            {
                throw new Exception($"Erro: Índice do array '{varName}' deve ser um inteiro, mas recebeu {indexObj.GetType().Name}.");
            }
            ...
            if (!(memory[varName] is object[] array)) // 🔹 Garante que a variável é um array
            {
                throw new Exception($"Erro: '{varName}' não é um array, então não pode ser acessado como '{varName}[{index}]'.");
            }

            if (index < 0 || index >= array.Length) // 🔹 Verifica limites do array
            {
                throw new Exception($"Erro: Índice {index} fora dos limites do array '{varName}'.");
            }
