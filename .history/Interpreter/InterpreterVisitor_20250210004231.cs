...
public override object VisitDeclaration(CSubsetParser.DeclarationContext context)
{
    ...
    object? value = null; // ✅ Agora a variável começa como `null`
    ...
    if (context.children.Count > 3 && context.GetChild(2).GetText() == "[") 
    {
        ...
        {
            array[i] = null; // ✅ Agora os arrays começam como `null`
        }
    ...
    memory[varName] = value; // ✅ Variáveis declaradas sem valor inicial permanecem `null`
    return null;
}
...
private void CheckInitialization(string varName)
{
    ...
    // ✅ Verifica se a variável foi declarada, mas nunca recebeu um valor válido
