...
public override object VisitPreprocessorDirective(CSubsetParser.PreprocessorDirectiveContext context)
{
    ...
    switch (directive)
    {
        case "include":
            ...
            {
                fileName = context.GetChild(2).GetText() + "." + context.GetChild(4).GetText(); // Formato `stdio.h`
            }
        ...
        case "define":
            string macroName = context.ID().GetText();
        ...
        case "error":
            string errorMessage = context.STRING().GetText().Trim('"');
            throw new Exception($"❌ Erro definido pelo usuário: {errorMessage}");
