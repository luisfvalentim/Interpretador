...
{
    // Executa a inicialização do for (ex: int i = 0; ou i = 0;)
    if (context.declaration() != null)
    {
        Visit(context.declaration()); // Se houver uma declaração de variável
    }
    else if (context.assignmentExpression(0) != null)
    {
        Visit(context.assignmentExpression(0)); // Se for uma atribuição (ex: i = 0;)
    }
    ...
    {
        Visit(context.block()); // Executa o bloco do for
