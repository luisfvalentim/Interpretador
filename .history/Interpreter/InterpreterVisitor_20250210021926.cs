...
public override object VisitPrimary(CSubsetParser.PrimaryContext context)
{
    ...
    else if (context.ID() != null)
    {
        ...
        if (context.LBRACKET() != null) // 🔹 Verifica presença de colchetes `[]`
        {
    ...
    if (functionName == "itoa")
{
    ...
    // ✅ Se o valor for um `float`, mas inteiro (ex: 42.0), converte para `int`
    if (value is float floatValue && floatValue == (int)floatValue)
    {
        return ((int)floatValue).ToString(); // ✅ Agora `itoa(42.0)` converte corretamente para "42"
    }

    // ❌ Se for um `float` real (ex: 42.5), gera erro
    if (value is float)
    {
