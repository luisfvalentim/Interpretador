...
public override object VisitIncludeDirective(CSubsetParser.IncludeDirectiveContext context)
{
    string fileName;

    if (context.STRING() != null) // ✅ Caso seja `#include "arquivo.h"`
    {
        fileName = context.STRING().GetText().Trim('"');
    }
    else // ✅ Caso seja `#include <stdio.h>`
    {
        string baseName = context.ID(0).GetText();
        string extension = context.ID(1) != null ? context.ID(1).GetText() : "h"; // ✅ Se não houver extensão, assume `.h`
    ...
    Visit(includedTree); // ✅ Processa o arquivo incluído
    return null;
}
