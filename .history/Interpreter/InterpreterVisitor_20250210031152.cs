...
public override object VisitPreprocessorDirective(CSubsetParser.PreprocessorDirectiveContext context)
{
    ...
    switch (directive)
    {
        case "include":
            ...
            // ✅ Verifica se a inclusão usa `#include "arquivo.h"` ou `#include <arquivo.h>`
        ...
        case "error":
            ...
            {
                throw new Exception($"❌ Erro: Diretiva `#error` requer uma string como mensagem.");
            }

            string errorMessage = context.STRING().GetText().Trim('"');
            throw new Exception($"❌ Erro de pré-processamento: {errorMessage}");
