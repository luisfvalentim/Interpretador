...
public override object VisitPrimary(CSubsetParser.PrimaryContext context)
{
    ...
    else if (context.ID() != null)
    {
        ...
        // 🔹 Verifica se há um acesso ao array `arr[i]`
        if (context.expression().Length > 0) // ✅ Agora garantimos que há um índice dentro de `[]`
        {
            object indexObj = Visit(context.expression(0)); // ✅ Obtém a expressão do índice corretamente

            if (!(indexObj is int index)) // ✅ Agora garantimos que o índice seja sempre inteiro
            {
            ...
            if (!(memory[varName] is object[] array)) // ✅ Verifica corretamente se é um array
            {
                throw new Exception($"Erro: '{varName}' não é um array.");
            }
