...
{
    // 1️⃣ Inicialização de variáveis no escopo do loop
    ...
    while (context.expression() == null || Convert.ToBoolean(Visit(context.expression())))
    {
        try
        {
            ...
            // 4️⃣ Atualiza as variáveis de controle
            if (context.assignmentExpression().Length > 1)
            {
                for (int i = 1; i < context.assignmentExpression().Length; i++)
                {
                    Visit(context.assignmentExpression(i));
                }
            }
        }
        catch (BreakException)
