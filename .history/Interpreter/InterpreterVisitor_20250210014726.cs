...
public override object VisitPrimary(CSubsetParser.PrimaryContext context)
{
    ...
    else if (context.ID() != null)
    {
        ...
        if (context.GetChildCount() > 3 && context.GetChild(1).GetText() == "[") // ✅ Verifica presença de colchetes `[]`
        {
            // ✅ Obtém corretamente o índice dentro dos colchetes
            object indexObj = Visit(context.GetChild(2)); 

            // 🔹 Garante que o índice seja inteiro antes de acessar o array
            if (!(indexObj is int index))
            {
