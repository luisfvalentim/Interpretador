        ...
        public override object VisitFunctionCall(CSubsetParser.FunctionCallContext context)
{
    ...
    if (functionName == "printf")
    {
        ...
        int argIndex = 0;
        while ((formatString.Contains("%d") || formatString.Contains("%f") || formatString.Contains("%c")) && argIndex < args.Count)
        {
            if (formatString.Contains("%c") && args[argIndex] is char)
            {
                formatString = formatString.Replace("%c", args[argIndex].ToString());
            }
            else if (formatString.Contains("%f"))
            {
                formatString = formatString.Replace("%f", string.Format("{0:F2}", args[argIndex]));
            }
            else if (formatString.Contains("%d"))
            {
                formatString = formatString.Replace("%d", args[argIndex].ToString());
            }
