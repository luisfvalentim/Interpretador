...
forStatement
    : 'for' '(' (declaration | assignmentExpression (',' assignmentExpression)*)? ';' 
               expression? ';' 
               (assignmentExpression (',' assignmentExpression)*)? ')' block
    ;
...
preprocessorDirective
    : '#' 'include' ('<' ID ('.' ID)? '>' | STRING) # IncludeDirective
    | '#' 'define' ID expression                   # DefineDirective
    | '#' 'error' STRING                           # ErrorDirective
    ;
